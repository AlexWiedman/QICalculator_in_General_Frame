...
##################### pyQSC
def plot_boundary(self, r=0.1, ntheta=80, nphi=150, ntheta_fourier=20, nsections=8, savefig=None, colormap=None, azim_default=None,
         show=True, **kwargs):
    """
    ...
    Decided to get rid of the fieldlines thing for the time being, since I don't have mayavi working on this computer.
...
##################### pyQSC
def plot_axis(self, nphi=100, frame=False, nphi_frame=80, frame_factor=0.12, savefig=None, show=True):
    '''
    ...
    All F-S frame stuff has been changed into a general curvilinear frame for this
    Once again keeping this False until mayavi can be worked out
    '''
    ...
    if frame:
        # Show frame vectors (THIS NEEDS TO BE FIXED, the spline stuff is still in the FS frame format)
        ...
        # Create array of toroidal angles where the curvilinear frame is shown
        phi_array = np.linspace(0, 2 * np.pi, nphi_frame)
        # Calculate origin and vector arrays for the frame
        R0 = self.R0_func(phi_array)
        Z0 = self.Z0_func(phi_array)
        x_plot = R0*np.cos(phi_array)
        y_plot = R0*np.sin(phi_array)
